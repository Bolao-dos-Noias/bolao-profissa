...
Coletado na primeira reconstrução limpa com `uv` e Python 3.12.
...
- `uv run radon cc app -s -a`: complexidade média A, 4.14.
- `uv run radon mi app -s`: todos os modulos em A.

Pontos de atenção do baseline:
...
- A cobertura inicial cobre os fluxos de autenticação, gating, ranking e snapshots incrementais, mas ainda precisa crescer em frontend, admin, sync ESPN real e seed.
- O score do pylint fica acima do gate de 9.0, apesar de avisos esperados de SQLModel e complexidade residual em rotas/servicos que devem ser alvo das próximas milestones.
- A complexidade média esta saudável, mas as funções listadas acima devem ser refatoradas conforme as tarefas de Caio e Gabriel.

## Rodada 2 - Lacunas funcionais da referência

Coletado após reaplicar a triagem do `bolao-dos-noia` sem editar a origem.
...
- `uv run radon cc app -s -a`: complexidade média A, 4.05.
- `uv run radon mi app -s`: todos os modulos em A.

Mudanças observadas:

- Cobertura subiu de 62% para 66% com testes de perfil, CSV, desempates, terceiros e palpites por jogo.
- `WorkflowGateMiddleware.__call__` caiu de C(20) para C(15) apos extração de helpers.
- `app/domain/classificacao.py::compute_standings` permanece D(21) e continua sendo o principal alvo técnico.
- Novas rotas de aposta ficaram controladas: `post_group_tiebreak` C(12), `post_thirds_order` B(7).

## Interpretação

- Complexidade ciclomática alta em `routers` indica regra de negócio vazando da camada de serviço.
- Maintainability index baixo em templates/CSS sugere duplicação ou UI difícil de revisar.
